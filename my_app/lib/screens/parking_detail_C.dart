...
class ParkingDetailCScreen extends StatefulWidget {
  const ParkingDetailCScreen({super.key});

  @override
  _ParkingDetailCScreenState createState() => _ParkingDetailCScreenState();
}

class _ParkingDetailCScreenState extends State<ParkingDetailCScreen> {
  final String lotId = 'C';
  ...
  String _lotName = 'ลาน C - อาคารตัวยู';
  String _lotLocation = 'ใกล้สวนป่ายาง';
  String _lotDescription = 'ลานจอดรถสำหรับผู้มาติดต่อราชการ';
  int _totalSpotsLive = 10;
  int _availableLive = 0;
  int _occupiedLive = 10;
  int? _lastUpdated;
  ...
  Widget _buildSpotsGridFromMap(
    ...
    // ignore: avoid_print
    print('C spot $key raw=${entry['occupied']} type=${entry['occupied']?.runtimeType} -> $occupied');
      final spotId = key;
      return GestureDetector(
        onTap: () async {
          if (!firebaseEnabled) {
